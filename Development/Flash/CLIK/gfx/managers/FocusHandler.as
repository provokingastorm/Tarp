	...
	public function handleInput(event:Object):Void {
		...
		if (path.length == 0 || path[0].handleInput == null || path[0].handleInput(event.details, path.slice(1)) != true) {
			...
			if (nav != null) {
				...
				trace("NewFocus's focusgroupmask: " + newFocus.focusGroupMask);
				trace("NewFocus's focusEnabled: " + newFocus.focusEnabled);
				trace("NewFocus's tabEnabled: " + newFocus.tabEnabled);
