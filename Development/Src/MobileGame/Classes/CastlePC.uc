...
class CastlePC extends MobilePC;
...
state PlayerTapToMove
{
	...
	event BeginState(Name PreviousStateName)
	{
		...
		{
			TutMenu.FadeOut();
			TutMenu = MobileMenuControls(MPI.OpenMenuScene(class'MobileGame.MobileMenuControls'));
			TutMenu.Setup(false);
...
function bool ProcessMenuSlide(MobileInputZone Zone, EZoneTouchEvent EventType, int SlideValue, Vector2D ViewportSizes)
{
	...
	if (EventType == ZoneEvent_UnTouch && !bPauseMenuOpen)
	{
...
exec function ShowSplash()
{
	...
	if (!bSplashHasBeenShown && !bSuppressSplash && MPI != none)
	{
		...
		MPI.OpenMenuScene(class'MobileGame.MobileMenuSplash');
	}
}
