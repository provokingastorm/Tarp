...
event UpdateSpawner( out CrowdSpawnInfoItem Item, float DeltaTime )
{
	local GameCrowdDestination PickedSpawnPoint;
	...
	if( Item.Remainder > 1.f )
	{
		...
		while( Item.Remainder > 1.f && Item.ActiveAgents.Length < Item.SpawnNum)
		{
			...
			if( PickedSpawnPoint != None )
			{
				...
				SpawnAgent( Item, PickedSpawnPoint);
				Item.Remainder -= 1.f;
			}
			else
			{
...
native function Warmup( out CrowdSpawnInfoItem Item, int WarmupNum );
